...
export default class Game extends Component {
    state = { dice1: null, dice2: null, current: 0, player1Turn: true, totalP1: 0, totalP2: 0 }
    ...
    changeTurnAndUpdate = (origin) => {
        ...
        this.setState(prev => {
            return { player1Turn: !prev.player1Turn, current: 0, [whoToAddTo]: prev[whoToAddTo] + amountToAddFromCurrent }
        })
    }
    ...
    render() {
        return (
            <div className="container" >
                <Player name="player1" totalScore={this.state.totalP1} currentScore={this.state.current} playerTurn={this.state.player1Turn} />
                <Controler changeTurn={this.changeTurnAndUpdate} diceA={this.state.dice1} diceB={this.state.dice2} onRollDiceClick={this.onRollDiceClick} />
                <Player name="player2" totalScore={this.state.totalP2} currentScore={this.state.current} playerTurn={!this.state.player1Turn} />
            </div>
        )
    }
}
...
function Player(props) {
    return (
        <div className="player">
            ...
            <div className="currentScore" > current: {props.playerTurn ? props.currentScore : 0}</div>
        </div >
    )
}
